...
import com.aerospike.client.{AerospikeException, Key, ResultCode, Value}
import com.aerospike.client.policy.{GenerationPolicy, RecordExistsAction, WritePolicy}
 ...
 */
class DefaultSource extends RelationProvider with Serializable with CreatableRelationProvider{
  ...
  private def savePartition(iterator: Iterator[Row],
    ...
    mode match {
      ...
      case SaveMode.Append => policy.recordExistsAction = RecordExistsAction.REPLACE
    }
