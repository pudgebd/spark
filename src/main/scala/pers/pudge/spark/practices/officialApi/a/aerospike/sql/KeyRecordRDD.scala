...
class KeyRecordRDD(
  ...
  val logger = LoggerFactory.getLogger(classOf[KeyRecordRDD])
...
class RowIterator[Row] (val kri: KeyRecordIterator, schema: StructType, config: AerospikeConfig, requiredColumns: Array[String] = null)
  extends Iterator[org.apache.spark.sql.Row] {

  val logger = LoggerFactory.getLogger(classOf[RowIterator[Row]])
