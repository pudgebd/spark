...
object WordBlacklist {
  ...
  def getInstances(sc: SparkContext): Broadcast[Seq[String]] = {
    if (instance == null) {
      synchronized {
        if (instance == null) {
          val workBlacklist = Seq("a", "b", "c")
          instance = sc.broadcast(workBlacklist)
        }
      }
    }
    instance
...
object RecoverableNetworkWordCount {
  ...
  def createContext(ip: String, port: Int, outputPath: String, checkpointDirectory: String): StreamingContext = {
    ...
    wordCounts.foreachRDD((rdd: RDD[(String, Int)], time: Time) => {
      ...
      val blacklist = WordBlacklist.getInstances(rdd.sparkContext)
      ...
      //                }
      //            }.collect().mkString("[", ",", "]")
      val counts = rdd.collect().mkString("[", ",", "]")
