...
object LouvainCore {
  ...
  def createLouvainGraph[VD: ClassTag](graph: Graph[VD, Long]): Graph[VertexState, Long] = {
    ...
    val louvainGraph = graph.outerJoinVertices(nodeWeights)((vid, vd, weightOption) => {
  ...
  def louvain(sc: SparkContext, graph: Graph[VertexState, Long], minProgress: Int = 1, progressCounter: Int = 1): (Double, Graph[VertexState, Long], Int) = {
    var louvainGraph = graph.persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    var msgRDD = louvainGraph.aggregateMessages[Map[(Long, Long), Long]](sendMsgNew, mergeMsg)
  ...
  private def sendMsgNew(triplet: EdgeContext[VertexState, Long, Map[(Long, Long), Long]]) = {
    triplet.sendToSrc(Map((triplet.dstAttr.community, triplet.dstAttr.communitySigmaTot) -> triplet.attr))
    triplet.sendToDst(Map((triplet.srcAttr.community, triplet.srcAttr.communitySigmaTot) -> triplet.attr))
  }
  ...
  /**
    ...
    */
  private def louvainVertJoin(
                               louvainGraph: Graph[VertexState, Long],
