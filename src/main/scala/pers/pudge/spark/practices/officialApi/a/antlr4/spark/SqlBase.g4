...
@header{package pers.pudge.spark.practices.officialApi.a.antlr4.spark;}
...
myCreateTableStmtE
 : myStmt=CREATE TEMPORARY? EXTERNAL? TABL (IF NOT EXISTS)? multipartIdentifier
   '(' columns=qualifiedColTypeWithPositionList ( ',' columns=qualifiedColTypeWithPositionList )*? ')'
   ((COMMENT tableComment=STRING) | LIFECYCLE lifecycleNum=INTEGER_VALUE)*
...
MONTHS: 'MONTHS';
MSCK: 'MSCK';
HIDDEN_CHANNEL: 'hiddenChannel' -> channel(HIDDEN);
NAMESPACE: 'NAMESPACE';
